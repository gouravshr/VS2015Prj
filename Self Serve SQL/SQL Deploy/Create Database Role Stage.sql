...
/****** Object:  Login [DIR\3371_SSS_App_Stage] ******/
IF NOT EXISTS (SELECT * FROM sys.syslogins WHERE name = 'DIR\3371_SSS_App_Stage')
BEGIN
	CREATE LOGIN [DIR\3371_SSS_App_Stage] FROM WINDOWS WITH DEFAULT_DATABASE=[master], DEFAULT_LANGUAGE=[us_english]
END
GO
...
-- Create the user
IF NOT EXISTS (SELECT * FROM sys.database_principals WHERE name = N'DIR\3371_SSS_App_Stage' AND type = 'U')
BEGIN
	CREATE USER [DIR\3371_SSS_App_Stage] FOR LOGIN [DIR\3371_SSS_App_Stage] WITH DEFAULT_SCHEMA=[dbo]
END
GO

exec sp_addrolemember '3371_SelfServeSql_Role', 'DIR\3371_SSS_App_Stage'
GO
